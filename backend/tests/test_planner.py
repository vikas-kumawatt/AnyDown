...
def test_max_height_is_enforced(youtube_info: dict) -> None:
    # 1440p exists in the fixture but max_height defaults to 1080.
    assert all((p.height or 0) <= 1080 for p in build_plans(youtube_info).values())
...
def test_missing_codec_fields_treated_as_progressive(tiktok_info: dict) -> None:
    plans = list(build_plans(tiktok_info).values())
    assert len(plans) == 1
